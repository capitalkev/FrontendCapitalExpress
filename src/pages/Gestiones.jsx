...
const Icon = ({ name, size = 16, ...props }) => {
  ...
  return (
    <LucideIcon
      size={size}
      {...props}
      className={`inline-block flex-shrink-0 ${props.className || ""}`}
    />
  );
};
...
const CardHeader = ({ children, className = "" }) => (
  <div className={`p-5 border-b border-gray-200/60 ${className}`}>
    {children}
  </div>
);
...
const Button = React.forwardRef(
  (
    ...
    const baseStyles =
      "inline-flex items-center justify-center gap-2 rounded-lg text-sm font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:opacity-60 disabled:pointer-events-none";
    const variants = {
      primary:
        "bg-red-500 text-white shadow-md hover:shadow-lg hover:bg-red-600 focus-visible:ring-red-500",
      secondary:
        "bg-blue-600 text-white shadow-sm hover:shadow-md hover:bg-blue-700 focus-visible:ring-blue-500",
      outline:
        "border border-gray-300 bg-white hover:bg-gray-100 text-gray-800 shadow-sm",
      success:
        "bg-green-600 text-white shadow-md hover:shadow-lg hover:bg-green-700 focus-visible:ring-green-500",
    ...
    return (
      <button
        ...
        {...props}
      >
        {iconName && <Icon name={iconName} size={16} className="mr-1" />}
        {children}
      </button>
    );
  }
          ...
          {notifications.map((notif) => (
            ...
            >
              <div className="flex items-start gap-3">
                <Icon
                  name={iconMap[notif.type]}
                  className={colorMap[notif.type]}
                  size={20}
                />
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <select
          ...
          className="h-10 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-red-500"
        >
          <option>Llamada</option>
          <option>Email Manual</option>
          <option>WhatsApp</option>
          <option>Visita en Terreno</option>
        </select>
        <select
          name="resultado"
          value={gestion.resultado}
          onChange={handleChange}
          className="h-10 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-red-500"
        >
          <option>Conforme</option>
          <option>No Contesta</option>
          <option>Discrepancia de Monto</option>
          <option>Desconoce Factura</option>
          <option>Solicita más tiempo</option>
        </select>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <input
          name="contacto"
          value={gestion.contacto}
          onChange={handleChange}
      ...
      <Textarea
        ...
        value={gestion.notas}
        onChange={handleChange}
        placeholder="Notas cualitativas de la gestión..."
      />
    ...
    {gestiones.length > 0 ? (
      <div className="max-h-48 overflow-y-auto space-y-2 pr-2">
        {gestiones.map((g, i) => (
          <div
            key={i}
            className="text-xs p-2 bg-gray-100 rounded-md border border-gray-200"
          >
            <p className="font-semibold">
              {g.tipo}:{" "}
              <span className="font-normal text-gray-600">{g.resultado}</span>
            </p>
            <p className="text-gray-500 italic">
              "{g.notas}" - {g.analista}
            </p>
            <p className="text-right text-gray-400">
              {new Date(g.fecha).toLocaleString("es-ES")}
            </p>
          </div>
        ))}
      </div>
      ...
      <p className="text-xs text-gray-500 italic">
        No hay gestiones manuales registradas.
      </p>
    )}
  </div>
);
      ...
      <div>
        ...
        <p className="text-blue-900/80">
          <strong className="text-blue-700">
            {deudor.contactoEfectivo.nombre}
          </strong>{" "}
          ({deudor.contactoEfectivo.cargo}) al{" "}
          <strong className="text-blue-700">
            {deudor.contactoEfectivo.telefono}
          </strong>
          .
        </p>
      </div>
        ...
        {facturas.map((factura) => (
          ...
          >
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={() => onCheck(factura.folio, "Rechazada")}
                title="Rechazar Factura"
                className="h-6 w-6 rounded-full hover:bg-red-200 text-red-500 flex items-center justify-center"
              >
                <Icon name="X" size={14} />
              </button>
              <button
                onClick={() => onCheck(factura.folio, "Verificada")}
                title="Verificar Factura"
                className="h-6 w-6 rounded-full hover:bg-green-200 text-green-500 flex items-center justify-center"
              >
  ...
  <Card>
    <CardHeader>
      <CardTitle className="flex items-center gap-2">
        <Icon name="CalendarCheck" className="text-red-500" />
        Verificaciones de la Semana
      </CardTitle>
    </CardHeader>
    <CardContent>
      <p className="text-3xl font-bold text-gray-900">
        {kpis.verificadasSemana}
      </p>
      <p className="text-sm text-gray-500">de {kpis.metaSemanal} operaciones</p>
      <ProgressBar
        value={kpis.verificadasSemana}
        max={kpis.metaSemanal}
  ...
  <Card>
    ...
    <CardContent className="text-sm text-center">
      ...
      <p className="font-semibold text-green-700">
        25% más rápido que tu promedio del mes pasado. ¡Excelente mejora!
      </p>
    </CardContent>
  </Card>
);
          ...
          {isSuccess ? (
            ...
            >
              ...
              <p className="text-gray-500">
                La operación {operation?.id} se ha movido a "Adelanto Express".
              </p>
            </motion.div>
          ) : (
            <motion.div key="form" exit={{ opacity: 0 }}>
              <p className="text-sm text-gray-600 mb-4">
                Estás a punto de mover la operación{" "}
                <strong className="text-gray-800">{operation?.id}</strong> a la
                cola de "Adelanto Express". Por favor, ingresa una justificación
                para esta acción.
              </p>
              <Textarea
                value={justification}
...
export default function Gestiones({ user, handleLogout, isAdmin = false }) {
  ...
  const fetchOperaciones = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:8000/api/gestiones/operaciones",
        {
          // Apunta a tu URL del orquestador
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errData.detail || "No se pudo obtener la data de gestiones."
        );
      }
  ...
  const deudorData = {
    "Comercial Andina": {
      tiempoPromedioRespuesta: "48 horas",
      contactoEfectivo: {
        nombre: "Juan Pérez",
        cargo: "Jefe de Tesorería",
        telefono: "998765432",
      },
      insight: "Suele responder después del 2do correo de seguimiento.",
    },
    "Mercados Frescos Inc.": {
      tiempoPromedioRespuesta: "4 horas",
      contactoEfectivo: {
        nombre: "Ana García",
        cargo: "Analista de Pagos",
        telefono: "pagos@mercadosfrescos.com",
      },
      insight: "Responde rápidamente a correos con el folio en el asunto.",
    },
    "Consorcio Minero": {
      tiempoPromedioRespuesta: "72+ horas",
      contactoEfectivo: {
        nombre: "Carlos Soto",
      ...
      insight:
        "Contacto difícil. Se recomienda llamar directamente a Gerencia.",
    },
    "Proyectos Urbanos": {
      ...
      insight: "Responde siempre a la primera llamada.",
    },
    "Comercializadora del Este": {
      tiempoPromedioRespuesta: "36 horas",
      contactoEfectivo: {
        nombre: "Luis Torres",
        cargo: "Contador",
        telefono: "987654123",
      },
      insight: "Prefiere contacto por email.",
    },
    "Minera Andina Central": {
      tiempoPromedioRespuesta: "48 horas",
      contactoEfectivo: {
        nombre: "Jorge Luna",
      ...
      insight: "A veces requiere reenvío de documentos.",
    },
    "Supermercados Nacionales": {
      tiempoPromedioRespuesta: "12 horas",
      contactoEfectivo: {
        nombre: "María Paz",
        cargo: "Pagos",
        telefono: "pagos@nacionales.com",
      },
      insight: "Proceso estandarizado y eficiente.",
    },
  };
  ...
  const logros = [
    {
      emoji: "🎯",
      titulo: "Verificador Experto",
      descripcion: "Completaste 20 verificaciones la semana pasada.",
    },
    {
      emoji: "⚡",
      titulo: "Rápido y Eficaz",
      descripcion: "Lograste un tiempo de respuesta promedio de 5h.",
    },
  ];
  const notifications = [
    {
      ...
      type: "success",
      message:
        "<b>Verificación Aprobada:</b> La operación <b>OP-00124</b> ha sido verificada.",
      time: "hace 5 minutos",
      read: false,
    },
    {
      ...
      type: "info",
      message:
        "<b>Respuesta Recibida:</b> El deudor de <b>OP-00125</b> ha respondido tu correo.",
      time: "hace 2 horas",
      read: false,
    },
    {
      ...
      type: "warning",
      message:
        "<b>Acción Requerida:</b> La operación <b>OP-00123</b> lleva 3 días sin respuesta.",
      time: "hace 1 día",
      read: true,
    },
  ];
  const filteredData = useMemo(() => {
    const enProceso = operaciones.filter(
      (op) => !op.adelantoExpress && op.estado !== "Completada"
    );
    const enAdelanto = operaciones.filter(
      (op) => op.adelantoExpress && op.estado !== "Completada"
    );
    switch (activeFilter) {
      case "En Proceso":
        return enProceso;
      case "Adelanto Express":
        return enAdelanto;
      default:
        return operaciones.filter((op) => op.estado !== "Completada");
    }
  ...
  const handleSaveGestion = async (opId, gestionData) => {
    try {
      const token = await user.getIdToken();
      const response = await fetch(
        `http://localhost:8000/api/operaciones/${opId}/gestiones`,
        {
          method: "POST",
          headers: {
      ...
      // En lugar de recargar todo, actualizamos solo la operación afectada
      await fetchOperaciones(); // O una actualización local más optimizada si la API devuelve la op completa
  ...
  const handleFacturaCheck = async (opId, folio, nuevoEstado) => {
    try {
      const token = await user.getIdToken();
      const response = await fetch(
        `http://localhost:8000/api/operaciones/${opId}/facturas/${folio}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      // El backend nos devuelve el nuevo estado de la operación
      const result = await response.json();

      // Actualizamos el estado local de forma inteligente
      setOperaciones((prevOps) =>
        prevOps.map((op) => {
          if (op.id === opId) {
            const nuevasFacturas = op.facturas.map((f) =>
              f.folio === folio
                ? { ...f, estado: result.nuevoEstadoFactura }
                : f
            );
            return {
              ...op,
              facturas: nuevasFacturas,
              estadoOperacion: result.nuevoEstadoOperacion,
            };
          }
          return op;
        })
  ...
  const handleCompleteVerification = (opId) => {
    setOperaciones((prevOps) =>
      prevOps.map((op) =>
        op.id === opId ? { ...op, estado: "Completada" } : op
      )
    );
    setTimeout(() => {
      ...
      setKpis((prevKpis) => ({
        ...prevKpis,
        verificadasSemana: prevKpis.verificadasSemana + 1,
      }));
      showPopup("¡Operación completada!");
    }, 300);
  };
  ...
  const handleConfirmAdelanto = (justification) => {
    setOperaciones((prevOps) =>
      prevOps.map((op) =>
        op.id === selectedAdelantoOp.id
          ? {
              ...op,
              adelantoExpress: true,
              gestiones: [
                ...op.gestiones,
                {
  ...
  const renderContent = () => {
    ...
    // 2. Muestra un error si la petición falló
    if (error) {
      return (
        <motion.div
          initial={{ opacity: 0 }}
    ...
    // 3. Muestra "Todo limpio" si no hay datos
    ...
    // 4. Muestra las operaciones si todo está bien
    return (
      <AnimatePresence>
        {filteredData.map((op) => (
          <OperationCard
        ...
        <main className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="p-0">
              <div className="space-y-4">
                {renderContent()}{" "}
                {/* <-- REEMPLAZA EL BLOQUE ANTERIOR CON ESTA LÍNEA */}
              </div>
            </CardContent>
          </Card>
        </main>
...
const OperationCard = ({
  ...
  const allInvoicesVerified = operation.facturas.every(
    (f) => f.estado === "Verificada"
  );
  const antiquity =
    Math.ceil(
      Math.abs(new Date() - new Date(operation.fechaIngreso)) /
        (1000 * 60 * 60 * 24)
    ) || 0;
  ...
  return (
    <Card>
      {operation.alertaIA && (
        ...
        >
          <Icon
            name={operation.alertaIA.tipo === "llamar" ? "PhoneCall" : "MapPin"}
            size={18}
          />{" "}
        ...
        <div className="grid grid-cols-10 gap-x-4 gap-y-4 items-center">
          ...
          <div className="col-span-12 md:col-span-5 grid grid-cols-3 gap-x-4 text-left">
            <div>
              ...
              <p className="font-semibold text-blue-600 whitespace-nowrap">
                {formatCurrency(
                  operation.facturas.reduce((sum, f) => sum + f.monto, 0),
                  operation.facturas[0]?.moneda || "PEN"
                )}
              </p>
            </div>
            <div>
          ...
          <div className="col-span-12 md:col-span-2 flex justify-start md:justify-end">
            {allInvoicesVerified ? (
              <Button
                variant="success"
                onClick={() => onCompleteVerification(operation.id)}
          ...
          >
            <div className="p-5 border-t border-gray-200/60 bg-gray-50/50">
              <Tabs defaultValue="gestion">
                <TabsList>
                  <TabsTrigger value="facturas" iconName="ListChecks">
                    Facturas (
                    {
                      operation.facturas.filter(
                        (f) => f.estado === "Verificada"
                      ).length
                    }
                    /{operation.facturas.length})
                  </TabsTrigger>
                  <TabsTrigger value="gestion" iconName="PlusCircle">
                    Registrar Gestión
                  </TabsTrigger>
                  <TabsTrigger value="historial" iconName="History">
                    Historial
                  </TabsTrigger>
                  <TabsTrigger value="ia" iconName="Lightbulb">
                    Asistencia IA
                  </TabsTrigger>
                </TabsList>
                <TabsContent value="gestion">
                  <GestionForm
                    onSave={(gestionData) =>
                      onSaveGestion(operation.id, gestionData)
                    }
                ...
                <TabsContent value="facturas">
                  <FacturaChecklist
                    facturas={operation.facturas}
                    onCheck={(folio, estado) =>
                      onFacturaCheck(operation.id, folio, estado)
                    }
                  />
                </TabsContent>
                ...
                <TabsContent value="ia">
                  <ContactoInteligente
                    deudor={{
                      ...deudorData[operation.deudor],
                      nombre: operation.deudor,
                    }}
                  />
                </TabsContent>
              </Tabs>
              <div className="mt-4 pt-4 border-t border-dashed border-gray-300">
                <Button
                  ...
                  size="sm"
                  iconName="Zap"
                  onClick={() => onOpenAdelantoModal(operation)}
                >
